...
-- Students can only view their own records with full details
...
-- Allow browsing of students but exclude sensitive contact information
-- This policy returns a subset of data for authenticated users who are not the owner
CREATE POLICY "Authenticated users can browse public student info" 
ON public.students 
FOR SELECT 
TO authenticated
USING (
  auth.uid() != user_id AND 
  -- This policy will be used by the application to filter columns
  true
);
