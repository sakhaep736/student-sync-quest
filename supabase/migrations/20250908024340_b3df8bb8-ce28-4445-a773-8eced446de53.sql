-- Create secure function for public student profiles (removes sensitive data)
...
-- Add validation trigger for students table
...
-- Add validation trigger for jobs table
...
-- Add validation trigger for contact requests
...
DROP TRIGGER IF EXISTS validate_job_data_trigger ON public.jobs;
...
CREATE TRIGGER validate_contact_request_trigger
  ...
  FOR EACH ROW EXECUTE FUNCTION public.validate_contact_request_data();

-- Update storage policies for better file security
INSERT INTO storage.objects (bucket_id, name, owner, metadata) 
VALUES ('profile-photos', '.keep', null, '{}') 
ON CONFLICT DO NOTHING;

-- Add file size and type restrictions to storage policies
DROP POLICY IF EXISTS "Profile photos size limit" ON storage.objects;
CREATE POLICY "Profile photos size limit" ON storage.objects
